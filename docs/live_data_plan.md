...
- **On-demand** — the Streamlit Storm Center calls the feeds server-side (no
  browser CORS issues), cached for 10 minutes so we're gentle on the APIs.
...
This is both honest and a stronger interview story than overclaiming: it shows
judgment about where ML is and isn't appropriate.
...
system. That closes the loop: the predictor is continuously *evaluated*, which is
exactly the platform's thesis.
